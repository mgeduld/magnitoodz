...
test('api:comparison:updateOne triggers queries and then submits a response', async (t: any) => {
  ...
  const updateOne = factory(
    apiDouble.api,
    userQueryDouble.query,
    comparisonQueryDouble.query
  )
  t.truthy(typeof updateOne === 'function', 'returned a function')
  ...
  t.is(calledForMethod, 'put', 'put method used')
  t.deepEqual(calledWithArg, { ok: true }, 'responded as expected')
})

test('api:comparison:updateOne fails due to bad userId', async (t: any) => {
  const apiDouble = apiDoubleFactory({
    body: {
  ...
  t.is(calledForMethod, 'put', 'post method used')
  t.deepEqual(
    nextCalledWith,
